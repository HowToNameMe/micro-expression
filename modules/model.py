...
class GeneratorFullModel(torch.nn.Module):
    ...
    def __init__(self, kp_extractor, bg_predictor, fg_predictor, dense_motion_network, inpainting_network, train_params, *kwargs):
        ...
        self.fg_predictor = None
        if fg_predictor:
            self.fg_predictor = fg_predictor
            self.fg_start = train_params['fg_start']
    ...
    def forward(self, x, epoch):
        kp_source = self.kp_extractor(x['source']) # bs KN 2
        kp_driving = self.kp_extractor(x['driving']) # bs KN 2
        bg_param = None
        fg_param = None
        if self.bg_predictor:
            if(epoch>=self.bg_start):
                bg_param = self.bg_predictor(x['source'], x['driving']) # affine matrix bs 3 3

        if self.fg_predictor:
            if(epoch>=self.fg_start):
                fg_param = self.fg_predictor(x['source'], x['driving']) # perspective matrix bs 3 3
        ...
        generated = self.inpainting_network(x['source'], x['source_mask'], dense_motion)
