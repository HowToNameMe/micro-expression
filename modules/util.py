...
class TPS:
    ...
    def transform_frame(self, frame):
        grid = make_coordinate_grid(frame.shape[2:], type=frame.type()).unsqueeze(0).to(frame.device) # w h 2
        grid = grid.view(1, frame.shape[2] * frame.shape[3], 2)
        shape = [self.bs, frame.shape[2], frame.shape[3], 2]
        if self.mode == 'kp':
            shape.insert(1, self.gs)
        grid = self.warp_coordinates(grid).view(*shape)
        return grid #实际上只用了frame的尺寸信息，bs gs w h 2
    ...
    def warp_coordinates(self, coordinates):
        ...
        return transformed # 原本的坐标在warp之后的坐标，bs w h 2
...
def kp2gaussian(kp, spatial_size, kp_variance):
    ...
    coordinate_grid = make_coordinate_grid(spatial_size, kp.type()).to(kp.device) # w h 2
    number_of_leading_dimensions = len(kp.shape) - 1
    shape = (1,) * number_of_leading_dimensions + coordinate_grid.shape # 把原来的[x,y]更换为 w h 2的一组坐标
    coordinate_grid = coordinate_grid.view(*shape)
    repeats = kp.shape[:number_of_leading_dimensions] + (1, 1, 1)
    coordinate_grid = coordinate_grid.repeat(*repeats) # 已经全部换完，现在每个wh2都对应之前的[x,y]
